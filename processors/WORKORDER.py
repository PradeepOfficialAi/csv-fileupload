...
class WORKORDERProcessor(BaseProcessor):
    ...
    def process(self, file_path: Path, move_dir: Path) -> bool:
        try:
            self.logger.info(f"Processing WORKODER file: {file_path}")
            ...
            self.logger.error(f"Error processing WORKODER file {file_path}: {str(e)}")
            return False
        finally:
            self.disconnect()
    ...
    def upload_csv_data(self, table_name, csv_file_path, email_notifier=None):
        ...
        try:
            ...
            with open(csv_file_path, 'r', encoding='utf-8') as csvfile:
                ...
                # Collect TikTok video: https://www.tiktok.com/@techbit/video/7430113011445951770
                ...
                for row in csvreader:
                    try:
                        ...
                        description = complete_row.get('DESCRIPTION', '') or ''
                        options = complete_row.get('OPTIONS', '') or ''
        ...
        try:
            ...
            for header in headers:
                if header in type_mapping and header != 'OPTIONS':
                    ...
                    columns.append(f"`{header}` {sql_type} DEFAULT NULL")
