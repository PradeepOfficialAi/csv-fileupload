...
class INVOICEDATESProcessor(BaseProcessor):
    ...
    def process(self, file_path: Path, move_dir: Path) -> bool:
        try:
            self.logger.info(f"Processing InvoiceDates file: {file_path}")
            ...
            self.logger.error(f"Error processing InvoiceDates file {file_path}: {str(e)}")
            return False
        finally:
            self.disconnect()

    def upload_csv_data(self, table_name, csv_file_path):
        """Upload CSV data to the invoice_dates table, updating or inserting based on OrderNumber"""
        ...
        try:
            ...
            headers = ['ORDER NUMBER', 'COMPANY', 'INVOICE DATE', 'INVOICE NUMBER', 'LIVE OR TEST', 'ORIGINAL ORDER#']
            ...
            # 4. Collect all rows and check OrderNumber against existing database
            rows_to_insert = []
            rows_to_update = []
            duplicates = []
            ...
            with open(csv_file_path, 'r', encoding='utf-8-sig') as csvfile:
                ...
                for row in csvreader:
                    try:
                        ...
                        if not order_number:
                            self.logger.warning(f"Skipping row with missing OrderNumber: {complete_row}")
                            continue

                        # Check if OrderNumber exists in database
                        cursor.execute(f"SELECT `ORDER NUMBER` FROM `{table_name}` WHERE `ORDER NUMBER` = %s", (order_number,))
            ...
            if rows_to_update:
                try:
                    update_columns = [h for h in headers if h != 'ORDER NUMBER']
                    set_clause = ', '.join([f"`{h}` = %s" for h in update_columns])
                    update_query = f"UPDATE `{table_name}` SET {set_clause} WHERE `ORDER NUMBER` = %s"
                    
                    for row in rows_to_update:
                        values = [row[h] for h in update_columns] + [row['ORDER NUMBER']]
            ...
            if duplicates:
                try:
                    #self.email_notifier.notify_duplicate(table_name, duplicates, 'order_number')
                    self.logger.info(f"Sent duplicate notification for {len(duplicates)} OrderNumber values")
                except Exception as e:
                    self.logger.error(f"Failed to send duplicate notification: {str(e)}")
                    for dup in duplicates:
                        self.logger.warning(f"Duplicate not notified: OrderNumber={dup['order_number']}, Order={dup['order']}")
