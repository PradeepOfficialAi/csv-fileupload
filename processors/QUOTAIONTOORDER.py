...
class QUOTAIONTOORDERProcessor(BaseProcessor):
    ...
    def process(self, file_path: Path, move_dir: Path) -> bool:
        try:
            ...
            self.logger.error(f"Error processing QUOTAIONTOORDER file {file_path}: {str(e)}")
            return False
        finally:
            self.disconnect()
    ...
    def upload_csv_data(self, table_name, csv_file_path):
        ...
        try:
            ...
            with open(csv_file_path, 'r', encoding='utf-8-sig') as csvfile:
                ...
                for row in csvreader:
                    try:
                        ...
                        cursor.execute(f"SELECT `TO_ORDER_NUMBER` FROM `{table_name}` WHERE `TO_ORDER_NUMBER` = %s", (order_number,))
            ...
            if rows_to_update:
                try:
                    update_columns = [h for h in headers if h != 'TO_ORDER_NUMBER']
                    set_clause = ', '.join([f"`{h}` = %s" for h in update_columns])
                    update_query = f"UPDATE `{table_name}` SET {set_clause} WHERE `TO_ORDER_NUMBER` = %s"
                    
                    for row in rows_to_update:
                        values = [row[h] for h in update_columns] + [row['TO_ORDER_NUMBER']]
