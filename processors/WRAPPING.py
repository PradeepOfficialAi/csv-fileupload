...
class WRAPPINGProcessor(BaseProcessor):
    ...
    def process(self, file_path: Path, move_dir: Path) -> bool:
        try:
            self.logger.info(f"Processing WRAPPING file: {file_path}")
            ...
            self.logger.error(f"Error processing WRAPPING file {file_path}: {str(e)}")
            return False
        finally:
            self.disconnect()

    def upload_csv_data(self, table_name, csv_file_path):
        """Upload CSV data to the wrapping table, checking LineNumber for duplicates"""
        ...
        try:
            ...
            has_headers = False
            first_line_headers = []
            with open(csv_file_path, 'r', encoding='utf-8-sig') as csvfile:  # Handle BOM
                first_line = csvfile.readline().strip()
                first_line_headers = [h.strip() for h in first_line.split(',')]
                # Remove BOM, quotes, and normalize headers
            ...
            # 4. Collect all rows and check LineNumber against existing database
            rows_to_insert = []
            duplicates = []
            
            with open(csv_file_path, 'r', encoding='utf-8-sig') as csvfile:
                csvreader = csv.DictReader(csvfile, fieldnames=headers)  # Explicitly set fieldnames to avoid quotes/BOM
                ...
                for row in csvreader:
                    try:
                        ...
                        if not line_number:
                            self.logger.warning(f"Skipping row with missing LineNumber: {complete_row}")
                            continue
            ...
            # 5. Check LineNumber values against existing database
            if rows_to_insert:
                line_numbers = [row['LineNumber'] for row in rows_to_insert]
                format_strings = ','.join(['%s'] * len(line_numbers))
                query = f"SELECT `LineNumber` FROM `{table_name}` WHERE `LineNumber` IN ({format_strings})"
                cursor.execute(query, line_numbers)
                existing_line_numbers = {row[0] for row in cursor.fetchall()}
                self.logger.debug(f"Existing LineNumber values in database: {existing_line_numbers}")
            ...
            if duplicates:
                try:
                    self.email_notifier.notify_duplicate(table_name, duplicates, 'LineNumber')
                    self.logger.info(f"Sent duplicate notification for {len(duplicates)} LineNumber values")
                except Exception as e:
                    self.logger.error(f"Failed to send duplicate notification: {str(e)}")
                    for dup in duplicates:
                        self.logger.warning(f"Duplicate not notified: LineNumber={dup['line_number']}, Order={dup['order']}")
