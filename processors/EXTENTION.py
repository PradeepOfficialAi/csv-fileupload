...
class EXTENTIONProcessor(BaseProcessor):
    ...
    def process(self, file_path: Path, move_dir: Path) -> bool:
        try:
            self.logger.info(f"Processing EXTENTION file: {file_path}")
            ...
            self.logger.error(f"Error processing EXTENTION file {file_path}: {str(e)}")
            return False
        finally:
            self.disconnect()
